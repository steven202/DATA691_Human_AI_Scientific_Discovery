...
def evaluate_model_on_dataset(
    ...
    preds_df = df[["doc_tokens", "label"]].copy()
    preds_df["pred_label"] = pred_labels
    preds_df["pred_prob"] = [p if isinstance(p, (int, float)) else (p[1] if len(p) > 1 else p[0]) for p in raw_probs]
    ...
    for model_name in args.models:
        ...
        for dataset_name, df in loaded_datasets.items():
            ...
            with open(out_path, "w") as f:
                result_clean = {k: v for k, v in result.items() if k != "_predictions_df"}
                json.dump(result_clean, f, indent=2)
            print(f"[evaluate] Saved: {out_path}")

            # Save per-sample predictions for re-binning analysis
            preds_df = result.get("_predictions_df")
            if preds_df is not None:
                preds_path = out_path.with_suffix(".predictions.csv")
                preds_df.to_csv(preds_path, index=False)
                print(f"[evaluate] Saved per-sample predictions: {preds_path}")
